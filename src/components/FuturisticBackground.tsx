...
const FuturisticBackground: React.FC<{ className?: string; withParticles?: boolean }> = ({
  ...
  return (
    ...
    >
      ...
      {/* Gradient orbs */}
      <div
        className="absolute w-[600px] h-[600px] rounded-full blur-[120px] opacity-20 -top-1/4 -left-1/4"
        style={{ background: "radial-gradient(circle, rgba(0, 245, 255, 0.4) 0%, transparent 70%)" }}
      />
      <div
        className="absolute w-[500px] h-[500px] rounded-full blur-[100px] opacity-15 bottom-0 right-0 translate-y-1/3 translate-x-1/3"
        style={{ background: "radial-gradient(circle, rgba(14, 165, 233, 0.5) 0%, transparent 70%)" }}
      />
      <div
        className="absolute w-[400px] h-[400px] rounded-full blur-[80px] opacity-10 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
